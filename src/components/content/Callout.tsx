...
export function Callout({ tip = "nota", titlu, children, className }: CalloutProps) {
  ...
  return (
    // Pe telefon caseta e cât un ecran de lată și orice rând în plus se vede:
    // spațiul din jur se strânge acolo, nu și pe desktop.
    <aside
      className={cn("flex gap-3 rounded-lg border p-4 sm:gap-3.5 sm:p-5", stil.clase, className)}
    >
      ...
      <div className="min-w-0">
        ...
        <div className="text-text-slab mt-1.5 text-base sm:text-lg [&_a]:underline">{children}</div>
      </div>
    </aside>
  );
}
